...
while cap.isOpened(): #鏡頭是否能開啟
    ...
    if results.multi_hand_landmarks: #如果有找到手部
        for i in range(len(results.multi_handedness)): #所有的手
            ...
            finger=[0,0,0,0,0]
            if thisHandLMList[4][1] > thisHandLMList[3][1] :
                finger[0]=1
            if thisHandLMList[8][2] < thisHandLMList[7][2]:
                finger[1]=1
            if thisHandLMList[12][2] < thisHandLMList[11][2]:
                finger[2]=1
            if thisHandLMList[16][2] < thisHandLMList[15][2]:
                finger[3]=1
            if thisHandLMList[20][2] < thisHandLMList[19][2]:
                finger[4]=1
            print(finger)
